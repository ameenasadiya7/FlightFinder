...
mongoose.connect(process.env.MONGODB_URI
    ...
    app.post('/add-flight', async (req, res)=>{
        ...
        try{
            ...
            const newFlight = await flight.save(); //here i change await

            res.json({message: 'flight added'});

        }catch(err){
            console.log("Error in adding flight",err);//here i change this line
            res.status(500).json({message: 'failed to add flight',error:err});//this also chnange
            //console.log(err);
        }
    })
...
//To run the code through the github
